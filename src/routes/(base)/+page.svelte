...
<div class="right-side pl-5 border-l-2 border-surface-200" in:fade={{ delay: 300, duration: 300 }}>
    ...
    <div class="todo-container pt-4 grid grid-cols-1 gap-2" in:fade|global={{ delay: 300, duration: 300 }}>
        ...
        {#each data.assignments as { course_number, assignment_name, active, due_date, color, date_color, course_id, assignment_id }}
            {#if active === true && date_color === 'crimson'}
                <TaskComponent {course_number} {assignment_name} {due_date} {color} {date_color} {course_id} {assignment_id} />
            {/if}
        {/each}
